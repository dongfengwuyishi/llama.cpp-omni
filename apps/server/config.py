...
_EXAMPLE_PATH = os.path.join(_SERVER_DIR, "config.example.json")
...
class AudioConfig(BaseModel):
    ...
    ref_audio_path: Optional[str] = Field(
        ...
        description="默认参考音频路径（TTS 声音克隆，相对于 apps/ 目录）",
    )
...
class ServiceSectionConfig(BaseModel):
    ...
    worker_base_port: int = Field(
        default=22700,
        description="Worker 起始端口（Worker 0 = 22700, Worker 1 = 22701, ...）",
    )
...
class CppBackendConfig(BaseModel):
    ...
    llamacpp_root: str = Field(
        ...
        description="llama.cpp-omni 项目根目录（默认自动检测为仓库根目录）",
    )
    model_dir: str = Field(
        default="",
    ...
    llm_model: str = Field(
        default="",
        description="LLM GGUF 文件名（留空则自动检测，优先 Q8_0，其次 Q4_K_M / Q4_K_S / F16）",
    )
    ...
    vision_backend: str = Field(
        default="auto",
        description="Vision 编码器后端: 'auto'(macOS 有 .mlmodelc 则用 CoreML/ANE，否则 Metal), 'metal', 'coreml'",
    )
...
def load_config(path: str = _CONFIG_PATH) -> ServiceConfig:
    ...
    # ref_audio_path 如果是相对路径，基于 apps/ 目录解析
    if config.audio.ref_audio_path and not os.path.isabs(config.audio.ref_audio_path):
        resolved = os.path.join(_APPS_ROOT, config.audio.ref_audio_path)
        if os.path.exists(resolved):
            config.audio.ref_audio_path = resolved

    logger.info(
        f"配置已加载: backend={config.backend}, "
        f"gateway_port={config.gateway_port}, "
        f"playback_delay_ms={config.playback_delay_ms}, "
        f"ref_audio={config.audio.ref_audio_path}"
    )
    return config
