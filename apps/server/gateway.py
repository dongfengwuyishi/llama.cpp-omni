...
启动方式：
    ...
    python gateway.py --workers localhost:22700
"""
...
_APPS_ROOT = os.path.dirname(_SERVER_DIR)
_REPO_ROOT = os.path.dirname(_APPS_ROOT)
_FRONTEND_DIR = os.path.join(_APPS_ROOT, "frontend")
_ASSETS_DIR = os.path.join(_APPS_ROOT, "assets")
_CERTS_DIR = os.path.join(_APPS_ROOT, "certs")


_SESSION_ID_RE = re.compile(r'^[a-zA-Z0-9_\-]+$')

# ============ Build fingerprint ============
_BUILD_INFO_PATH = os.path.join(_APPS_ROOT, "build_info.json")
_BUILD_INFO: dict = {}
if os.path.isfile(_BUILD_INFO_PATH):
    try:
...
async def lifespan(app: FastAPI):
    ...
    # 初始化参考音频注册表
    data_dir = os.path.join(_SERVER_DIR, "data", "assets", "ref_audio")
...
app = FastAPI(
    ...
    version=_BUILD_INFO.get("version", "1.0.0-alpha.2"),
    lifespan=lifespan,
)


# ============ Build fingerprint middleware ============

from starlette.middleware.base import BaseHTTPMiddleware

class _BuildHeaderMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        response = await call_next(request)
        if _BUILD_INFO:
            response.headers["X-Comni-Build"] = _BUILD_INFO.get("build_commit", "dev")
        return response

app.add_middleware(_BuildHeaderMiddleware)
...
@app.get("/version")
async def version():
    """Build information and integrity fingerprint."""
    if not _BUILD_INFO:
        return {"app_name": "Comni", "version": "dev", "note": "development mode"}
    safe = {k: v for k, v in _BUILD_INFO.items() if k != "integrity"}
    return safe
...
async def duplex_ws(ws: WebSocket, session_id: str):
    ...
    try:
        ...
        if worker:
            duration = (datetime.now() - task_start).total_seconds() if task_start else 0
            # Duplex 结束后 Worker 可能执行 C++ full_reinit，期间不可被重新分配
...
def _load_presets_from_dir(project_root: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    presets_root = os.path.join(_ASSETS_DIR, "presets")
...
async def get_presets():
    ...
    _presets_cache = _load_presets_from_dir(_APPS_ROOT)
    return _presets_cache
...
async def get_preset_audio(mode: str, preset_id: str):
    ...
    if "system_content" in preset:
        ...
        for item in preset["system_content"]:
            if item.get("type") == "audio" and item.get("path"):
                loaded = _load_audio_base64(item["path"], _APPS_ROOT)
    ...
    if preset.get("ref_audio") and preset["ref_audio"].get("path"):
        loaded = _load_audio_base64(preset["ref_audio"]["path"], _APPS_ROOT)
...
async def get_default_ref_audio():
    ...
    if not os.path.isabs(ref_path):
        ref_path = os.path.join(_APPS_ROOT, ref_path)
...
async def session_viewer(session_id: str):
    ...
    viewer_path = os.path.join(_FRONTEND_DIR, "session-viewer.html")
...
def main():
    ...
    parser.add_argument("--ssl-certfile", type=str,
                        default=os.path.join(_CERTS_DIR, "cert.pem"), help="SSL cert file path")
    parser.add_argument("--ssl-keyfile", type=str,
                        default=os.path.join(_CERTS_DIR, "key.pem"), help="SSL key file path")
