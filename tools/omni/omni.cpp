...
bool stream_prefill(struct omni_context * ctx_omni, std::string aud_fname, std::string img_fname, int index, int max_slice_nums, std::string text) {
    ...
    //
    // 注：duplex 路径下用户输入都是音频/图像，text 参数（web-demo 引入的
    // turn-based text-in，给 simplex chat 文本输入用）在此处不透传。
    ...
    if (index == 0 && !ctx_omni->system_prompt_initialized) {
        ...
        // 🔧 [契约说明] index=0 这条分支**只负责** system prompt 初始化：
        //   - aud_fname 在此处被当作 ref_audio（如果非空）；非空时已经在
        //     上面的 system prompt 初始化里被 prefill 过；空时回退到
        //     ctx_omni->ref_audio_path 或默认 ref_audio。
        //   - img_fname 和 text 参数在这条路径上**会被静默丢弃**——既不会
        //     被加进 KV cache，也不会被 push 到 llm_thread queue。
        //   - 因此 Python 调用方（cpp_backend.chat / chat_prefill / duplex_*）
        //     如果想喂用户内容，必须保证：
        //       (a) system_prompt_initialized 已经为 true（比如先发一次
        //           index=0 的占位 prefill），然后用户 content 从 index>=1
        //           起步；
        //       (b) 或者在 update_session_config 里通过 voice_audio 字段触发
        //           一次 stream_prefill(index=0) 完成系统初始化。
        //   绕开这条契约会让首条用户消息悄无声息地丢掉，下游表现就是
        //   "模型自由生成 / 返回空文本 / RL logits 只捕获到 system prefill 部分"。
        //
        // 历史背景：之前 cpp_backend.chat() 把首条用户内容丢在 cnt=0，正是
        // 因为踩到了这条契约 → 触发 server: add NO_TTS / --no-tts mode 提交里
        // 的 known limitations（/v1/chat broken / logits incomplete）。已通过
        // Python 侧先发占位 prefill 修复。
        print_with_timestamp("stream_prefill(index=0): system prompt 初始化完成，ref_audio 已在其中 prefill；本调用的 img/text 参数被忽略\n");
...
bool stream_decode(struct omni_context * ctx_omni, std::string debug_dir, int round_idx) {
    ...
    //
    // [Defense in depth] 历史上只在 duplex_mode 下 reset；simplex 路径在
    // use_tts=true 时由 T2W 线程顺手清掉 break_event，use_tts=false 没有 T2W
    // 兜底，于是 update_session_config（内部 POST /v1/stream/break）之后再发
    // /v1/stream/decode，sampling loop 看到 break_event=true 直接退出。无论
    // 双工/单工/use_tts 真假都在这里显式 reset，是廉价的兜底。
    // 注意：use_tts=false 下 /v1/chat 真正的 "返回空 text" 根因不在这里——
    // 而是 chat() 在 cnt=0 把首条用户 content 喂进 stream_prefill 时，C++
    // 恰好在 index=0 走 "system prompt 初始化" 分支并静默丢弃用户内容（见
    // 下方 stream_prefill 注释）。Python 侧已通过先发一次 index=0 占位调用
    // 让 system prompt 单独初始化、user content 从 index>=1 起步来修复。
    if (ctx_omni->break_event.load()) {
        ctx_omni->break_event.store(false);
