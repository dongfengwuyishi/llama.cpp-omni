...
# 顺序跑 prefill+generate+finalize，最后聚合返回。不流式输出。
#
# 与 /ws/duplex 的区别：
#   - 单次 HTTP 请求 / 单次 HTTP 响应（无 pause/resume/interrupt）
#   - 不接 session_recorder（评测/刷数据场景不需要回放）
#   - 不接背压队列（client 已经把完整数据交过来了，不会再追加）
#   - chunk_idx 由 server 控制，client 不感知
#
# 调用方应通过 batch_server 的 FIFO 队列控制并发，与现有 /chat 一致。
...
async def duplex_offline(request_body: Dict[str, Any]):
    ...
    if not worker.state.is_idle:
        for _ in range(10):
            ...
            raise HTTPException(
                status_code=429,
                detail=f"Worker busy, status: {worker.state.status.value}",
            )

    try:
        req = DuplexBatchRequest(**request_body)
    except Exception as e:
    ...
    worker.state.status = WorkerStatus.DUPLEX_ACTIVE
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    worker.state.current_session_id = f"{ts}_batch_{req.request_id or uuid.uuid4().hex[:8]}"
    t0_total = time.perf_counter()

    temp_files: List[str] = []

    def _decode_audio_to_16k_mono(
        path: Optional[str], b64: Optional[str]
    ) -> np.ndarray:
        """统一把输入音频解码成 16kHz mono float32 numpy"""
        if path:
            ...
            if sr != 16000:
                try:
                    ...
                    num = int(round(len(data) * 16000 / sr))
                    data = ssig.resample(data, num).astype(np.float32)
                except ImportError:
                    raise RuntimeError(
                        f"Input audio sr={sr} but scipy not available for resample"
                    )
        ...
        raise ValueError("user audio missing: need user_audio_path or user_audio_base64")

    def _resolve_ref_audio() -> Optional[str]:
        """决定 LLM ref audio path（可能是临时文件）"""
        if req.ref_audio_path:
            return req.ref_audio_path
        if req.ref_audio_base64:
            ...
            try:
                ...
                sr = 16000
            tmp = tempfile.NamedTemporaryFile(
                suffix=".wav", delete=False, prefix="duplex_offline_ref_"
            )
            sf.write(tmp.name, data, sr)
            temp_files.append(tmp.name)
            return tmp.name
        # 按语种走默认
        lang = _infer_lang_from_texts([req.system_prompt])
        return _LANG_REF_AUDIO_PATHS.get(lang) or _LANG_REF_AUDIO_PATHS.get("zh")

    chunks_out: List[DuplexChunkResult] = []
    ...
    try:
        # ---------- 解码音频 + 切片 ----------
        full_audio = _decode_audio_to_16k_mono(
            req.user_audio_path, req.user_audio_base64
        )
        if req.leading_silence_ms > 0:
            pad = np.zeros(
                int(16000 * req.leading_silence_ms / 1000), dtype=np.float32
            )
            full_audio = np.concatenate([pad, full_audio])
        chunk_samples = int(16000 * req.config.chunk_ms / 1000)
        n_chunks = int(np.ceil(len(full_audio) / chunk_samples))
        if req.max_chunks is not None:
            n_chunks = min(n_chunks, req.max_chunks)

        # ---------- 准备 per-chunk 图像帧 ----------
        per_chunk_frames: List[Optional[List]] = [None] * n_chunks
        image_list_paths = req.image_paths or []
        if image_list_paths:
            for i in range(n_chunks):
                idx = min(i, len(image_list_paths) - 1)
                per_chunk_frames[i] = [Image.open(image_list_paths[idx])]
        elif req.image_base64_list:
            b64s = req.image_base64_list
            for i in range(n_chunks):
                idx = min(i, len(b64s) - 1)
                frame_bytes = base64.b64decode(b64s[idx])
                per_chunk_frames[i] = [Image.open(_io.BytesIO(frame_bytes))]

        # ---------- duplex prepare ----------
        ref_audio_path = _resolve_ref_audio()
        prompt_wav_path = req.prompt_wav_path or ref_audio_path
        lang = _infer_lang_from_texts([req.system_prompt])
        media_type = 2 if any(f is not None for f in per_chunk_frames) else 1
        sampling = _build_cpp_sampling(req.config.model_dump())
        ...
        # ---------- 主循环：prefill → generate → finalize ----------
        for chunk_idx in range(n_chunks):
            t_chunk = time.perf_counter()
            start = chunk_idx * chunk_samples
            end = min(start + chunk_samples, len(full_audio))
            audio_chunk = full_audio[start:end]
            if len(audio_chunk) < chunk_samples:
                audio_chunk = np.concatenate(
                    [
                        audio_chunk,
                        np.zeros(chunk_samples - len(audio_chunk), dtype=np.float32),
                    ]
                )

            frame_list = per_chunk_frames[chunk_idx]

            def _step(_audio=audio_chunk, _frames=frame_list):
                prefill_result = worker.duplex_prefill(
                    audio_waveform=_audio,
                    frame_list=_frames,
                    max_slice_nums=req.max_slice_nums,
                )
                gen_result = worker.duplex_generate(force_listen=False)
                # 立刻 finalize，保证下一个 chunk 的 KV 状态一致
                try:
                    worker.duplex_finalize()
                except Exception:
                    pass
                return gen_result, prefill_result

            try:
                result, prefill_cost = await asyncio.to_thread(_step)
            except Exception as e:
                logger.error(
                    f"[duplex_offline] chunk {chunk_idx} failed: {e}", exc_info=True
                )
                stopped_reason = "error"
                return DuplexBatchResponse(
                    success=False,
                    error=str(e),
                    chunks=chunks_out,
                    full_text="".join(text_pieces),
                    total_chunks=chunk_idx,
                    speak_chunks=speak_n,
                    listen_chunks=listen_n,
                    audio_duration_s=sum(len(a) for a in audio_pieces) / 24000.0,
                    total_duration_ms=(time.perf_counter() - t0_total) * 1000,
                    stopped_reason=stopped_reason,
                    request_id=req.request_id,
                ).model_dump()

            # Collect audio. PyTorch backend returns audio_data inline on
            # ``result``. The C++ backend writes WAV files asynchronously to
            # ``tools/omni/output_<port>/<round>/tts_wav/`` and exposes them via
            # ``_collect_wav_output_nowait`` (incremental, returns new bytes
            # since last call). We poll once per chunk so we don't miss them.
            elapsed_ms = (time.perf_counter() - t_chunk) * 1000
            chunk_audio_b64: Optional[str] = result.audio_data
            if (
                chunk_audio_b64 is None
                and not result.is_listen
                and hasattr(worker, "_collect_wav_output_nowait")
            ):
                try:
                    polled_b64, _ = await asyncio.to_thread(
                        worker._collect_wav_output_nowait
                    )
                    if polled_b64:
                        chunk_audio_b64 = polled_b64
                except Exception as e:
                    logger.warning(f"[duplex_offline] WAV poll failed: {e}")

            has_audio_b64 = bool(chunk_audio_b64)
            if has_audio_b64:
                chunk_audio_np = np.frombuffer(
                    base64.b64decode(chunk_audio_b64), dtype=np.float32
                )
                audio_pieces.append(chunk_audio_np)
            if not result.is_listen and result.text:
            ...
            if req.include_text_timeline:
                chunks_out.append(
                    DuplexChunkResult(
                        chunk_idx=chunk_idx,
                        phase=("response" if not result.is_listen else "user"),
                        is_listen=result.is_listen,
                        text=result.text or "",
                        has_audio=has_audio_b64,
                        audio_data=(
                            chunk_audio_b64 if req.return_per_chunk_audio else None
                        ),
        ...
        # ---------- Tail flush for C++ backend: TTS writes WAV asynchronously,
        # so after the last chunk a few hundred ms of audio may still be in
        # flight. Poll a couple of times to drain it.
        if hasattr(worker, "_collect_wav_output_nowait"):
            for _ in range(8):
                try:
                    polled_b64, _ = await asyncio.to_thread(
                ...
                if polled_b64:
                    audio_pieces.append(
                        np.frombuffer(base64.b64decode(polled_b64), dtype=np.float32)
                    )
                await asyncio.sleep(0.25)

        # ---------- 合成 merged audio ----------
        merged_b64 = None
        if req.return_merged_audio and audio_pieces:
        ...
        # duplex_stop + 完整重置（与 /ws/duplex 一致）
        try:
            await asyncio.to_thread(worker.duplex_stop)
        except Exception:
            pass
        worker.state.status = WorkerStatus.LOADING
        try:
            ...
            logger.error(f"[duplex_offline] cleanup failed: {e}", exc_info=True)
            worker.state.status = WorkerStatus.ERROR
        worker.state.current_session_id = None
        worker.state.total_requests += 1
        logger.info(
            f"[duplex_offline] done req_id={req.request_id} "
